...
std::shared_ptr<open3d::geometry::PointCloud> BuildPcdFromRgbd(
    ...
    if (voxel_size <= 0.0) {
        if (estimate_normals && !pcd->points_.empty()) {
            pcd->EstimateNormals(geometry::KDTreeSearchParamHybrid(0.002 * 2, 30));
        }
        return pcd;
    }
...
std::pair<Eigen::Matrix4d, Eigen::Matrix6d> RunIcp(
    ...
    const open3d::pipelines::registration::ICPConvergenceCriteria icp_crit(
        1e-6, 1e-6, GetRegistrationParams().icp_max_iterations);
...
std::optional<Eigen::Matrix4d> FeatureRegistrationRgb(
    ...
    return MatchRansacStyle(A, B, GetRegistrationParams().marker_match_rmse_tol_meters);
}
...
std::optional<Eigen::Matrix4d> MarkerRegistration(
    ...
    int diag_src_frame,
    int diag_dst_frame) {
    const int hw = GetRegistrationParams().marker_corner_depth_half_window;
    const int min_v = GetRegistrationParams().marker_corner_min_valid_samples;
    auto diag_prefix = [&]() -> std::string {
        if (!GetRegistrationParams().marker_registration_diagnostics) {
            return {};
        }
    ...
    const bool diag_this_pair =
        GetRegistrationParams().marker_registration_diagnostics &&
        (GetRegistrationParams().marker_registration_diag_verbose || diag_src_frame < 0 ||
         diag_dst_frame < 0 ||
    ...
    std::optional<Eigen::Matrix4d> opt =
        MatchRansacStyle(A, B, GetRegistrationParams().marker_match_rmse_tol_meters);
    if (diag_this_pair) {
        ...
        if (opt.has_value()) {
            ...
            std::cerr << pfx << "fail: MatchRansacStyle tol="
                      << GetRegistrationParams().marker_match_rmse_tol_meters
                      << " n_corr=" << src_pts.size()
